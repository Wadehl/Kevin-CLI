...
// pinia
import usePinia from "../addtional/pinia/index.mjs";
 ...
 * @return {Promise}
 */
const useBaseTemplate = async (base, target) => {
...
const generateTemplate = async (base, target) => {
  await useBaseTemplate(base, target);
  // 替换pinia相关文件
  if (base.includes("pinia")) {
    await usePinia(target, base.includes("ts"));
  }
  // if (base.includes("router")) {
  //   await useRouter(target);
  // }
};
